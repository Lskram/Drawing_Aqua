...
def _main_key(main: MainColor) -> Point:
    return (int(main.pos[0]), int(main.pos[1]))


def _shade_key(shade: ShadeButton) -> Point:
    return (int(shade.pos[0]), int(shade.pos[1]))


def _palette_key(main: MainColor, shade: ShadeButton) -> PaletteKey:
    return (_main_key(main), _shade_key(shade))
...
def _select_shade(
    ...
    if last_main is None or _main_key(main) != _main_key(last_main):
    ...
    if last_shade is None or _shade_key(shade) != _shade_key(last_shade):
...
def open_shades_panel_for_main(
    cfg: AppConfig,
    options: PainterOptions,
    main: MainColor,
) -> None:
    """Open the shade panel for a main color using the same clicks as painting."""
...
def select_shade_for_painting(
    cfg: AppConfig,
    options: PainterOptions,
    main: MainColor,
    shade: ShadeButton,
) -> None:
    """Select a shade exactly as the paint worker does.

    This is used by the setup wizard so captured palette positions are tested
    with the same click sequence that later paints the image.
    """
...
def _verify_and_repair_row(
    ...
    for _pass in range(max_passes):
        ...
        groups: Dict[PaletteKey, Tuple[MainColor, ShadeButton, List[int]]] = {}
...
def paint_grid(
    ...
    resume_base_bucket_key: Optional[PaletteKey] = None,
    resume_base_bucket_rgb: Optional[RGB] = None,
    bucket_base_cb: Optional[Callable[[str, int, int, int, int, int, int, int], None]] = None,
    progress_cb: Optional[Callable[[int, int], None]] = None,
    should_stop: Optional[Callable[[], bool]] = None,
    status_cb: Optional[Callable[[str], None]] = None,
    verify_cb: Optional[Callable[[Optional[Tuple[int, int]]], None]] = None,
    ...
    bucket_key: Optional[PaletteKey] = None
    if allow_bucket_fill and bool(getattr(cfg, "bucket_fill_enabled", False)):
        # Build usage counts.
        counts: Dict[PaletteKey, Tuple[int, MainColor, ShadeButton]] = {}
    ...
    for y in range(grid_h):
        ...
        while x < grid_w:
            ...
            if bucket_key is not None and _palette_key(main, shade) == bucket_key:
...
def _paint_grid_by_color(
    ...
    # Group by real palette button identity. User-entered color names are not
    # guaranteed unique, and grouping by name can paint with the wrong main color.
    groups: Dict[PaletteKey, Tuple[MainColor, ShadeButton, List[Tuple[int, int]]]] = {}
    ...
    if allow_bucket_fill and bool(getattr(cfg, "bucket_fill_enabled", False)) and ordered:
        ...
        if len(coords0) >= min_cells:
            ...
            bucket_key = _palette_key(main0, shade0)
